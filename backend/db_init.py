...
def init_database():
    ...
    # Create simple users table (if schema didn't include it)
    try:
    ...
    try:
        ...
        c.execute("INSERT OR IGNORE INTO users (email, name, password_hash, is_admin) VALUES (?, ?, ?, ?)",
                  ('admin@example.com', 'Administrator', pw, 1))
        print('Inserted default user: admin@example.com / password')
    except Exception as e:
        print(f'Could not insert default user: {e}')
